...

{
    ...
    my @e = @{ $@->errors() };
    is( $e[0]->{message}, q{A user must have a username or email address.},
...

{
    ...
    ok( $@, 'cannot create a new user without a password (or setting disable_login to true)' );
    can_ok( $@, 'errors' );

    my @e = @{ $@->errors() };
    is( $e[0]->{message}, q{A user requires a password.},
        'got expected error message' );
}
