...

{
    ...
    is( $provider->add_uri('bubba'), 'msnim:add?contact=bubba',
        'add_uri() for provider' );

    is( $provider->status_uri('bubba'), 'http://mystatus.skype.com/mediumincon/bubba',
...

{
    ...
    is( $provider->add_uri('bubba'), undef,
...

{
    ...
    is( $provider->status_uri('bubba'), 'http://api.oscar.aol.com/presence/icon?k=abcdefg123456&t=bubba',
...

{
    ...
    like( $@, qr/No value for config key: EmptyKey/,
          'error when config key is empty' );

    eval { $provider->chat_uri('bubba') };
    like( $@, qr/Invalid config key: NoSuchKey/,
