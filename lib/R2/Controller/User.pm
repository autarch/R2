    ...
    sub _authenticate_openid {
        ...
        unless ($identity) {
            ...
            $c->redirect_with_error(
                error => $error,
                uri   => $c->domain()
                    ->application_uri( path => '/user/login_form' ),
        ...
        my $return_to = $c->domain()->application_uri(
...
sub openid_authentication : Local {
    ...
    unless ($user) {
        ...
        $c->redirect_with_error(
            error =>
                'Now you need to create a Rapport account for your OpenID URL',
            uri => $c->domain()
                ->application_uri( path => '/user/new_user_form' ),
